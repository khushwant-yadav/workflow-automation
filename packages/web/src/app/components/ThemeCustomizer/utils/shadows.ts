...
export const getShadowMap = (
  ...
  const createColorWithOpacity = (baseOpacity: number) => {
    ...
    if (colorFormat === 'oklch') {
      ...
      const oklchValues =
        formattedColor.match(/oklch\((.*?)\)/)?.[1] || '0 0 0';
  ...
  const shadowMap: { [key: string]: string } = {
    ...
    'shadow-2xs': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      0.5,
    )}`,
    'shadow-xs': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      0.5,
    )}`,
    'shadow-2xl': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      2.5,
    )}`,

    // Two layer shadows - use base vars for layer 1, mix fixed/calculated for layer 2
    'shadow-sm': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      1.0,
    )}, ${secondLayer('1px', '2px')}`,
    shadow: `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      1.0,
    )}, ${secondLayer('1px', '2px')}`,
    'shadow-md': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      1.0,
    )}, ${secondLayer('2px', '4px')}`,
    'shadow-lg': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      1.0,
    )}, ${secondLayer('4px', '6px')}`,
    'shadow-xl': `${offsetX} ${offsetY} ${blur} ${spread} ${createColorWithOpacity(
      1.0,
